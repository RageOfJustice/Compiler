...
public class Lexi {
    ...
    /**
     * Функция обрабатывает символы, пропуская его через ДКА, образует токен и возвращает его.
     * @return Следующий токен
     * @throws Exception
     */
    public Token getNextToken() throws Exception {
        //TODO решить проблему с определением переменных в switch/case
                ...
                case 1:
                    if (symb == ' ' || symb == '\t'|| symb == '\r') break;
                        ...
                        if(getLastAddedToken().getTag()==Tag.IDF ||
                                getLastAddedToken().getTag()==Tag.IF ||
                                getLastAddedToken().getTag()==Tag.CYCLE){
                            isTableOpened = true;
    ...
    public Lexi(File file) throws Exception {
        ...
        putKeyWord(new KeyToken(Tag.LOGIC,"logic"));
    ...
    public static void main(String args[]){
        ...
        try {
            // файл с программой
            lexi = new Lexi(new File("src\\main\\resources\\prog.txt"));
        } catch (Exception e) {
